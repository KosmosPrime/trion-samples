...
fn crc32(data: &[u8], mut crc: u32) -> u32
{
	...
	for &v in data
	{
		crc = (crc >> 8) ^ CRC_TABLE[usize::from(crc as u8 ^ v)];
	}
	!crc
}
...
fn test_crc()
{
	...
	assert_eq!(crc32(&[0x00], 0), 0xD202EF8D);
	assert_eq!(crc32(&[0xFF], 0), 0xFF000000);
	assert_eq!(crc32(b"The quick brown fox jumps over the lazy dog.", 0), 0x519025E9);
...
pub fn run_assembly_test(path: PathBuf, expect: u32, base: u32, pre_asm: fn(&mut Context))
{
	...
	let mut crc = 0u32;
	// we only check correctness of assembler output, trion is responsible for testing file formats
	for (range, data) in ctx.output().iter()
	{
		const SIZE_U32: usize = core::mem::size_of::<u32>();
		let mut range_enc = [0u8; 2 * SIZE_U32];
		range_enc[..SIZE_U32].copy_from_slice(&range.get_first().to_le_bytes());
		range_enc[SIZE_U32..].copy_from_slice(&range.get_last().to_le_bytes());
		crc = crc32(data, crc32(&range_enc, crc));
	}
	// not `assert_eq!()` because that uses decimal number format
	assert!(crc == expect, "incorrect assembly for {path:?} (expected 0x{expect:08X}, got 0x{crc:08X})");
}
